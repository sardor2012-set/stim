...
DATABASE_URL = os.environ.get(
    "RENDER_DB_URL",
    "postgresql://stim_user:otm7lBu27o0r2FZzyQy1ufq6YrPQbvdp@dpg-d7cumll7vvec73ejf840-a.oregon-postgres.render.com/stim_db_w8n5",
)
        ...
        keyboard=[
            [
                KeyboardButton(
                    ...
                    web_app=WebAppInfo(
                        url=f"{base_url}/?user_id={user_id}&name={quote(str(name or ''))}&photo={quote(str(photo_url or ''))}&v={int(time.time())}"
                    ),
                )
            ],
            [
            ...
            [
                KeyboardButton(
                    ...
                    web_app=WebAppInfo(
                        url=f"{base_url}/game?user_id={user_id}&username={quote(str(name or ''))}"
                    ),
                ),
                KeyboardButton(
                    text=" Yordam",
            ...
            [
                ...
                KeyboardButton(
                    ...
                    web_app=WebAppInfo(
                        url=f"{base_url}/game3?user_id={user_id}&username={quote(str(name or ''))}"
                    ),
                ),
            ],
        ],
...
def get_all_active_channels(force_refresh=False):
    ...
    if (
        force_refresh
        or _active_channels_cache is None
        or _cache_timestamp is None
        or (current_time - _cache_timestamp).total_seconds() > CACHE_DURATION
    ):
...
def init_db():
    with get_db() as conn:
        with conn.cursor() as db:
            ...
            db.execute(
                """CREATE TABLE IF NOT EXISTS game3_scores (id SERIAL PRIMARY KEY, user_id BIGINT NOT NULL, username TEXT, time_ms INTEGER NOT NULL, grid_size INTEGER DEFAULT 5, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"""
            )
            db.execute(
                """CREATE INDEX IF NOT EXISTS idx_game3_scores_user_id ON game3_scores(user_id)"""
            )
            db.execute(
                """CREATE INDEX IF NOT EXISTS idx_game3_scores_time ON game3_scores(time_ms ASC)"""
            )

            items_data = [
                (
                    1,
                    "Что означает этот знак + %s",
                    10,
                    "all",
                ...
                (2, "Сколько будет 2 + 2 * 2%s", 5, "all", "Математика", "4|6|8|0", 1),
                (
                    3,
                    "Столица Франции%s",
                    5,
                    "all",
                    "География",
                    "Берлин|Лондон|пАРИЖ|Рим",
                    2,
                ),
                (
                    4,
                    "Самая большая планета%s",
                    5,
                    "all",
                    "Астрономия",
                    "Марс|Земля|Юпитер Сатурн",
                    2,
                ),
                (5, "Химический символ золота%s", 5, "all", "Химия", "Ag|Au|Fe|Cu", 1),
            ]
...
class AntiSpamMiddleware(BaseMiddleware):
    ...
    async def __call__(self, handler, event, data):
        if isinstance(event, (Message, CallbackQuery)):
            ...
            user_message_timestamps[user_id] = [
                ts
                for ts in user_message_timestamps[user_id]
...
def rate_limit_ip(limit: int = FLASK_RATE_LIMIT, window: int = FLASK_RATE_WINDOW):
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            ip_request_timestamps[client_ip] = [
                ts
                for ts in ip_request_timestamps[client_ip]
...
def game3():
    user_id = request.args.get("user_id")
    username = request.args.get("username", "")
    return render_template("game3.html", user_id=user_id, username=username)
...
@app.route("/api/game3/score", methods=["POST"])
@rate_limit_ip(limit=20, window=60)
def save_game3_score():
    try:
        data = request.get_json()
        user_id  = data.get("user_id")
        username = data.get("username", f"User {user_id}")
        ...
        if not user_id or time_ms is None or time_ms <= 0:
            return jsonify({"error": "Missing required fields"}), 400

        with get_db() as conn:
            with conn.cursor() as db:
                db.execute(
                    "INSERT INTO game3_scores (user_id, username, time_ms, grid_size) VALUES (%s, %s, %s, %s)",
                    (user_id, username, time_ms, grid_size),
                )
                conn.commit()

                db.execute(
                    "SELECT MIN(time_ms) as best_time FROM game3_scores WHERE user_id = %s AND grid_size = %s",
                    (user_id, grid_size),
                )
                best_time = db.fetchone()["best_time"] or time_ms
                is_new_record = (best_time >= time_ms)
        ...
        logger.error(f"Error saving game3 score: {e}")
        return jsonify({"error": str(e)}), 500


@app.route("/api/game3/leaderboard")
@rate_limit_ip(limit=50, window=60)
def get_game3_leaderboard():
    try:
        grid_size = request.args.get("grid_size", 5, type=int)

        with get_db() as conn:
            with conn.cursor() as db:
                db.execute("""
                    SELECT gs.user_id,
                           COALESCE(NULLIF(TRIM(u.first_name || ' ' || COALESCE(u.last_name, '')), ''), gs.username) as username,
                           MIN(gs.time_ms) as best_time_ms,
                           COUNT(*) as games_played
                    FROM game3_scores gs
                    LEFT JOIN users u ON gs.user_id = u.user_id
                    WHERE gs.grid_size = %s
                    GROUP BY gs.user_id, gs.username, u.first_name, u.last_name
        ...
        logger.error(f"Error getting game3 leaderboard: {e}")
        return jsonify({"error": str(e)}), 500
    ...
    try:
        ...
        with get_db() as conn:
            with conn.cursor() as db:
                ...
                league, league_place, league_rank, top_players = (
                    "Boshlang'ich liga",
                    "Hammasi",
                    None,
                    [],
                ...
                if class_name:
                    ...
                    if clean_class:
                        ...
                        if 1 <= class_num <= 4:
                            league, league_place, league_classes = (
                                "Boshlang'ich liga",
                                "1-4 sinf",
                                [
                                    "1",
                ...
                if league_classes:
                    ...
                    clean_league_nums = [
                        "".join(filter(str.isdigit, c))
                        for c in league_classes
                    ...
                    count_query += (
                        ") AND (rating > %s OR (rating = %s AND user_id < %s))"
                    )
                    count_params = (
                        list(league_classes)
                    ...
                    for idx, lu in enumerate(league_top, 1):
                        p_name = (
                            f"{lu['first_name']} {lu['last_name']}"
                            if lu["first_name"]
                            else (lu["username"] or f"User {lu['user_id']}")
                        )
                        top_players.append(
                            {
                                "user_id": lu["user_id"],
                                ...
                                "rating": lu["rating"],
                                "rank": idx,
                                "photo": get_user_row(lu, "photo_url"),
                            }
                        )
                else:
                    db.execute(
                    ...
                    all_leagues_tops[l_name] = [
                        {
                            ...
                            if t["first_name"]
                            else (t["username"] or f"User {t['user_id']}"),
                            "rating": t["rating"],
                            "photo": get_user_row(t, "photo_url"),
                        }
                        for t in tops
                    ]
                season_days = 30
                ...
                return jsonify(
                    {
                        ...
                        "is_registered": bool(
                            get_user_row(user, "is_registered", False)
                        ),
                        "rating": rating,
                        "referrals": rating,
                        "status": "Boshlang'ich"
                        if rating < 50
                        else ("O'rganuvchi" if rating < 100 else "Master"),
                        "league": league,
                        "league_place": league_place,
                        "league_rank": league_rank,
                        "display_name": display_name,
                        "photo": photo_url,
                        "is_admin": str(user_id) in [str(id) for id in ADMIN_IDS],
...
def register_user():
    try:
        data = request.json
        user_id, first_name, last_name, class_name = (
            data.get("user_id"),
            data.get("first_name"),
            data.get("last_name"),
            data.get("class_name"),
    ...
    try:
        ...
        with get_db() as conn:
            with conn.cursor() as db:
                ...
                if user_id:
                    db.execute(
                        "SELECT task_id FROM user_tasks WHERE user_id = %s", (user_id,)
                    )
                    ...
                    tasks_list.append(
                        {
                            ...
                            "options": it["options"].split("|"),
                            "completed": it["item_id"] in completed_tasks,
                        }
                    )
...
def get_user_tasks(user_id):
    try:
        with get_db() as conn:
            with conn.cursor() as db:
                db.execute(
                    "SELECT class_name FROM users WHERE user_id = %s", (user_id,)
                )
                user = db.fetchone()
                            ...
                            completed_list.append(
                                {
                                    "task_id": task_id,
                                    "name": bundle["name"],
                                    "earned_rating": row["earned_rating"] or 0,
...
def complete_tasks():
    try:
        ...
        correct_count, incorrect_count = (
            ...
            data.get("incorrect_count", 0),
        )

        if user_id is None:
            return jsonify(
                {"success": False, "message": "Missing or invalid user_id"}
            ), 400
...
def start_bundle():
    try:
        ...
        if not user_id or not task_id:
            return jsonify(
                {"success": False, "message": "Missing user_id or task_id"}
            ), 400
...
async def admin_command(message: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Ручная рассылка", callback_data="admin_broadcast_manual"
                )
            ],
            [
                InlineKeyboardButton(
                    text="Рассылка о новых задачах",
                    callback_data="admin_broadcast_tasks",
                )
            ],
...
async def admin_broadcast_tasks(callback: CallbackQuery):
    ...
    await callback.message.answer(
        "Янги вазифалар рассылкаси бошланди...", parse_mode="HTML"
    )
        ...
        for user in users:
            try:
                await bot.send_message(
                    user["user_id"], text, reply_markup=keyboard, parse_mode="HTML"
                )
                count += 1
                await asyncio.sleep(0.05)
            except Exception:
...
async def cmd_start(message: Message):
    ...
    try:
        ...
        with get_db() as conn:
            with conn.cursor() as db:
                db.execute(
                    "SELECT is_blocked FROM users WHERE user_id = %s", (user.id,)
                )
                db_user = db.fetchone()
...
async def check_and_notify_subscription(
    user_id, message: Message, checking_msg: Message
):
    ...
    try:
        ...
        if not is_subscribed:
            ...
            await message.answer(
                channels_text, parse_mode="HTML", reply_markup=channels_keyboard()
            )
        else:
...
async def menu_about(message: Message):
    ...
    await message.answer(
        about_text, parse_mode="HTML", reply_markup=main_menu_keyboard_no_webapp()
    )
    ...
    try:
        ...
        for idx, user in enumerate(top_users, 1):
            name = (
                ...
                if user["first_name"]
                else (user["username"] or f"User {user['user_id']}")
            )
        ...
        if len(rating_text) > 4000:
            for part in [
                rating_text[i : i + 4000] for i in range(0, len(rating_text), 4000)
            ]:
                await message.answer(
                    part, parse_mode="HTML", reply_markup=main_menu_keyboard_no_webapp()
                )
        else:
            await message.answer(
                rating_text,
        ...
        await message.answer(
            "❌ Reytingni olishda xatolik yuz berdi.",
            reply_markup=main_menu_keyboard_no_webapp(),
        )
...
async def menu_help(message: Message):
    ...
    await message.answer(
        support_text, parse_mode="HTML", reply_markup=main_menu_keyboard_no_webapp()
    )
